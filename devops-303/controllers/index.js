...
async function indexHandler(request) {
  ...
  const [totalSeen, ownSeenCount] = redis
    ? await Promise.all([
  ...
  const perContainerSeenCount = redis
    ? await redis.hgetall("perContainerSeenCount")
    : 0;
  ...
  return `<html><pre>
    ...
    ---
    Container name\t | Visit count${Object.entries(perContainerSeenCount)
      .map(([name, count]) => `\n\t${name}\t | ${count}`)
      .join("")}
  `;
}
